...
class SpeedCalculator(Node):
    def __init__(self):
        ...
        # ROS2 publisher
        self.speed_publisher = self.create_publisher(Float64, 'wheel_speed', 10)

        # ROS2 timer to publish speed at a fixed rate (e.g., 10 Hz)
        self.timer = self.create_timer(0.005, self.publish_speed)
    ...
    def calculate_speed_continuously(self):
        while rclpy.ok():  # Continue running as long as ROS2 is active
            ...
            # Timeout check: If no rising edge is detected within the timeout duration, set speed to zero
            if time.time() - self.last_time > self.timeout_duration:
                self.current_speed_kmh = 0.0
